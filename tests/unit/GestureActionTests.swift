...
class GestureActionTests: XCTestCase {
  func testThatDraggableDrags() {
    ...
    pan.simulateTouch(state: .began)
    pan.simulateTouch(translation: CGPoint(x: 25, y: -50), state: .changed)
  ...
  func testThatPinchableScales() {
    ...
    pinch.simulateTouch(scale: 1, state: .began)
    pinch.simulateTouch(scale: 0.50, state: .changed)
  ...
  func testThatRotatableRotates() {
    let rotateGesture = SimulatedRotationGestureRecognizer()
    ...
    rotateGesture.simulateTouch(state: .began)
    rotateGesture.simulateTouch(rotation: CGFloat.pi / 2, state: .changed)
